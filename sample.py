    ...
    if explore_data == "overall_stats":
        ...
        if select_box == "Transaction":
            ...
            with tab1:
                ...
                with col1:
                    ...
                    cursor.execute(
                        f"select state_name, sum(total_count) as Total_Transactions, sum(amount) as Total_amount from map_trans_state where year = {select_year} and quater = '{select_quater}' group by state_name order by state_name")
                    df1 = pd.DataFrame(cursor.fetchall(), columns=['State', 'Total_Transactions', 'Total_amount'])
                    df2 = pd.read_csv('C:/Users/91962/Downloads/Statenames.csv')
                    df1.State = df2
                ...
                with col4:
                    ...
                    b1 = col5.button("States")
                    b2 = col6.button("Districts")
                    b3 = col7.button("Pincode")
                    if b1:
                        cursor.execute(
                            f"select state_name, state_total_count  from top_trans_overall where year = {select_year} and quater = '{select_quater}' ")
                        ...
                        cursor.execute(
                            f"select districts_name, districts_total_count  from top_trans_overall where year = {select_year} and quater = '{select_quater}' ")
                        df = pd.DataFrame(cursor.fetchall(), columns=['State', 'Transactions_Count'])
                        st.dataframe(df, hide_index=True)
                        fig = px.pie(df, values='Transactions_Count',
                                     names='State',
                        ...
                        cursor.execute(
                            f"select pincode, pincode_total_count  from top_trans_overall where year = {select_year} and quater = '{select_quater}' ")
                        df = pd.DataFrame(cursor.fetchall(), columns=['State', 'Transactions_Count'])
                        st.dataframe(df, hide_index=True)
                        fig = px.pie(df, values='Transactions_Count',
                                     names='State',
                ...
                with tab2:
                    ...
                    with col4:
                        ...
                        if a1:
                            cursor.execute(
                                f"select state_name, state_amount from top_trans_overall where year = {select_year} and quater = '{select_quater}' ")
                            ...
                            cursor.execute(
                                f"select districts_name, districts_amount  from top_trans_overall where year = {select_year} and quater = '{select_quater}' ")
                            ...
                            cursor.execute(
                                f"select pincode, pincode_amount  from top_trans_overall where year = {select_year} and quater = '{select_quater}' ")
        ...
        if select_box == "User":
            ...
            with tab1:
                col1, col2 = st.columns([2, 1])
                cursor.execute(
                    f"(select state_name, round(avg(app_open)) as app_opener, sum(total_count) as total_reg from agg_user_state where year = {select_year} and quater ='{select_quater}' group by state_name order by state_name)")
                a = pd.DataFrame(cursor.fetchall(), columns=["state_name", "app_opener", "total_reg"])
                with col2:
                    col2.markdown("## :violet[Users]")
                    users = a['total_reg'].sum()
                    col2.markdown(
                        f"#### :violet[Registered PhonePe users till {select_quater[:2]} {select_year}]")
                ...
                with col1:
                    ...
                    cursor.execute(
                        f"(select state_name, round(avg(appopen)) as app_opener, sum(reg) as total_reg from map_user_state where year = {select_year} and quater ='{select_quater}' group by state_name order by state_name)")
                    df1 = pd.DataFrame(cursor.fetchall(),
                                       columns=['State', 'Total_App_openers', 'Total_Reg_user'])
                    df2 = pd.read_csv('C:/Users/91962/Downloads/Statenames.csv')
                    df1.State = df2
                    fig = px.choropleth(df1,
                                        geojson="https://gist.githubusercontent.com/jbrobst/56c13bbbf9d97d187fea01ca62ea5112/raw/e388c4cae20aa53cb5090210a42ebb9b765c0a36/india_states.geojson",
                ...
                with col4:
                    ...
                    if c1:
                        cursor.execute(
                            f"select state_name,state_reg from top_user_overall where year ={select_year} and quater ='{select_quater}'")
                        ...
                        cursor.execute(
                            f"select districts_name,districts_reg from top_user_overall where year ={select_year} and quater ='{select_quater}'")
                        ...
                        cursor.execute(
                            f"select pincode,pincode_reg from top_user_overall where year ={select_year} and quater ='{select_quater}'")
                        ...
                        cursor.execute(
                            f"select brand,total_count from agg_user_overall where year ={select_year} and quater ='{select_quater}'")
        ...
        cursor.execute(
            f"select state_name from map_trans_state where year = {select_year} and quater = '{select_quater}' group by state_name order by state_name")
        ...
        if select_box == "Transaction":
            ...
            with tab1:
                col1, col2 = st.columns([2, 1])
                agg_trans = cursor.execute(
                    f"( select categories, sum(total_count) as total_counts, sum(amount) as total_amount from agg_trans_state where state_name = '{select_state}' and year = {select_year} and quater = '{select_quater}' group by categories order by categories);")
                a = pd.DataFrame(cursor.fetchall(), columns=["categories", 'total_counts', 'total_amount'])
                ...
                with col1:
                    ...
                    fig = px.bar(y=a["categories"],
                                 ...
                                 orientation="h",
                                 text=a["total_transaction_value"],
                                 labels={"y": "Categories", "x": "Total counts"},
                                 title="Total no transaction",
                                 color_discrete_sequence=px.colors.sequential.Agsunset)
                    fig.update_layout(title_x=0.4)
                ...
                agg_trans = cursor.execute(
                    f"(select district_name,total_count,amount from map_trans_state where state_name = '{select_state}' and year = {select_year} and quater = '{select_quater}');")
                b = pd.DataFrame(cursor.fetchall(), columns=["District_name", 'total_counts', 'total_amount'])
                b["total_transaction_value"] = b["total_counts"].apply(format_indian_number)
                fig = px.bar(x=b["District_name"],
                             y=b["total_counts"],
                             color=b["total_counts"],
                             orientation="v",
                             text=b["total_transaction_value"],
                ...
                with col4:
                    ...
                    if b1:
                        cursor.execute(
                            f"select districts_name, districts_total_count  from top_trans_state where state_name ='{select_state}' and year = {select_year} and quater = '{select_quater}' ")
                        ...
                        cursor.execute(
                            f"select pincode, pincode_total_count  from top_trans_state where state_name ='{select_state}' and year = {select_year} and quater = '{select_quater}' ")
            ...
            with tab2:
                ...
                with col1:
                    ...
                    a["total_transaction_value"] = a["total_amount"].apply(format_indian_number)
                    fig = px.bar(y=a["categories"],
                                 x=a["total_amount"],
                                 color=a["total_amount"],
                                 orientation="h",
                                 text=a["total_transaction_value"],
                                 labels={"y": "Categories", "x": "Total Amount"},
                                 title="Total No transaction Amount",
                                 color_discrete_sequence=px.colors.sequential.Agsunset)
                    fig.update_layout(title_x=0.4)
                    col1.plotly_chart(fig, use_container_width=True)
                ...
                agg_trans = cursor.execute(
                    f"(select district_name,total_count,amount from map_trans_state where state_name = '{select_state}' and year = {select_year} and quater = '{select_quater}');")
                b = pd.DataFrame(cursor.fetchall(), columns=["District_name", 'total_counts', 'total_amount'])
                b["total_transaction_value"] = b["total_amount"].apply(format_indian_number)
                fig = px.bar(x=b["District_name"],
                             y=b["total_amount"],
                ...
                with col4:
                    ...
                    if b1:
                        cursor.execute(
                            f"select districts_name, districts_amount  from top_trans_state where state_name ='{select_state}' and year = {select_year} and quater = '{select_quater}' ")
                        df = pd.DataFrame(cursor.fetchall(), columns=['Districts', 'Transactions_Amount'])
                        st.dataframe(df, hide_index=True)
                        fig = px.pie(df, values='Transactions_Amount',
                                     names='Districts',
                        ...
                        cursor.execute(
                            f"select pincode, pincode_amount  from top_trans_state where state_name ='{select_state}' and year = '{select_year}' and quater = '{select_quater}' ")
                        df = pd.DataFrame(cursor.fetchall(), columns=['Pincode', 'Transactions_Amount'])
                        st.dataframe(df, hide_index=True)
                        fig = px.pie(df, values='Transactions_Amount',
                                     names='Pincode',
            ...
            cursor.execute(
                f"(select round(avg(app_open)) as app_opener, sum(total_count) as total_reg from agg_user_state where state_name= '{select_state}' and year = '{select_year}' and quater ='{select_quater}' group by state_name order by state_name)")
            ...
            with col1:
                cursor.execute(
                    f"select district_name,reg, appopen from map_user_state where state_name= '{select_state}' and  year ='{select_year}' and quater ='{select_quater}'")
                df = pd.DataFrame(cursor.fetchall(),
                                  columns=["Districts", "Total_Registered_user", "Total_App_openers"])
            ...
            with col4:
                ...
                if c1:
                    cursor.execute(
                        f"select districts_name,districts_reg from top_user_state where state_name= '{select_state}' and  year ='{select_year}' and quater ='{select_quater}'")
                    df = pd.DataFrame(cursor.fetchall(), columns=["Districts", "Total_Registered_user"])
                    st.dataframe(df, hide_index=True)
                    fig = px.pie(df, values='Total_Registered_user',
                                 names='Districts',
                    ...
                    cursor.execute(
                        f"select pincode,pincode_reg from top_user_state where state_name= '{select_state}' and year ='{select_year}' and quater ='{select_quater}'")
                    df = pd.DataFrame(cursor.fetchall(), columns=["Pincode", "Total_Registered_user"])
                    st.dataframe(df, hide_index=True)
                    fig = px.pie(df, values='Total_Registered_user',
                                 names='Pincode',
                    ...
                    cursor.execute(
                        f"select brand,total_count from agg_user_state where state_name= '{select_state}' and  year ='{select_year}' and quater ='{select_quater}'")
                    df = pd.DataFrame(cursor.fetchall(), columns=["Brand", "Total_Registered_user"])
                    st.dataframe(df, hide_index=True)
                    fig = px.pie(df, values='Total_Registered_user',
                                 names='Brand',
...
if selected == "About":
    ...
    st.write("**:violet[My Project GitHub link - https://github.com/sheik-muzzammil/Phonepe-Project.git]** ⬇️")
